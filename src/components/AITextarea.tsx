  ...
  const scheduleFetch = useCallback(
    (text: string) => {
      ...
      timerRef.current = setTimeout(async () => {
        ...
        try {
          const result = await sendAIMessageWithFailover(
            getAllConfigs(),
            [
              {
                role: "system",
            ...
            // Inline autocomplete: keep it snappy and let the user's own
            // abort (next keystroke) cancel the request mid-flight.
            { timeoutMs: 15_000, signal: ctrl.signal },
          );
    ...
    [isFeatureOn, getAllConfigs, context],
  );
