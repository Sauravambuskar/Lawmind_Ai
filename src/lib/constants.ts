...
export const CASE_STATUSES = ["open", "in-progress", "closed", "won", "lost"] as const;
export type CaseStatus = (typeof CASE_STATUSES)[number];

export const CASE_STATUS_FILTER = ["all", ...CASE_STATUSES] as const;

export const CASE_STATUS_CONFIG: Record<CaseStatus, { bg: string; text: string; border: string }> = {
  "open":        { bg: "bg-blue-500/10",    text: "text-blue-600 dark:text-blue-500",    border: "border-blue-500/20" },
  "in-progress": { bg: "bg-amber-500/10",   text: "text-amber-600 dark:text-amber-500",  border: "border-amber-500/20" },
